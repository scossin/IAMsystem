...
public interface IOffsets {
  ...
  /**
   * End-offset is the index of the last character **+ 1**, that is to say the first character to
   * exclude from the returned substring when slicing with [start:end]
   *
   * @return end value.
   */
  public int end();
  ...
  public static String getSpanSeqId(Collection<? extends IOffsets> offsets) {
    return offsets.stream()
        .map(o -> String.format("(%d,%d)", o.start(), o.end()))
