...
public class PrefixTrie {
	...
	public PrefixTrie(int minPrefixLength) {
		...
		this.trie = new Trie();
		this.charTokenizer = new TokenizerImp(NormFunctions.rmAccents,
				SplitFunctions.splitChar);
	...
	public void addToken(Collection<String> tokens) {
		tokens.stream().filter(w -> !tokenLengthLessThanMinSize(w))
		.forEach((w) -> addToken(w));
	}
